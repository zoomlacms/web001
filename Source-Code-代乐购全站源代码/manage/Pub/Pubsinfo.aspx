<%@ page language="C#" autoeventwireup="true" inherits="manage_Pub_Pubsinfo, App_Web_sczrssus" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<body>
    ...
    <div class="r_navigation">
        ...
        <span>后台管理</span> &gt;&gt; 
            <asp:Label ID="Label1" runat="server" Text="Label"></asp:Label>管理</span>
    ...
    <asp:GridView ID="Egv" runat="server" AllowPaging="True" CssClass="border" AutoGenerateColumns="False" DataKeyNames="ID" PageSize="20" OnRowDataBound="Egv_RowDataBound" OnPageIndexChanging="Egv_PageIndexChanging" OnRowCommand="Lnk_Click" Width="100%">
        <Columns>
            ...
            <HeaderStyle BorderColor="White"  HorizontalAlign=Center/>
                <ItemTemplate>
                <a href="ViewSmallPub.aspx?Pubid=<%= this.HiddenPubid.Value %>&ID=<%# Eval("ID", "{0}")%><%= this.HiddenNode.Value %> ">
                  相关信息与回复管理</a>
                    |
               <a href="AddPub.aspx?Pubid=<%#this.HiddenPubid.Value%>&Parentid=<%#Eval("ID") %><%= this.HiddenNode.Value %>"  >回复</a>&nbsp;
                    |
